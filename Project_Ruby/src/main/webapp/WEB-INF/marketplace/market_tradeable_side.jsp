...
$(function(){
	...
	$("#changebox").change(function(){
		...
		{
			location.href = '/marketplace/sidesearchtest?&subtitle=${subtitle}&colorradio=${colorradio}&marketprice=${marketprice}';
		}
...
<div class="container">
	...
	<%--전체 테이블 --%>
	<c:forEach var="a" items="${TradeSideList}">
		  	...
		  	<div class="sangpumphoto" style="border: 0px solid #dbdbdb;">
				...
				<c:if test="${a.photo!='no'}">
					<c:forTokens var="p" items="${a.photo}" delims="," begin="0" end="0">
						<a href="${root }/marketplace/productdetail?subtitle=${subtitle}&colorradio=${colorradio}&marketprice=${marketprice}&market_place_idx=${a.market_place_idx}&currentPage=${currentPage}&checkside=1">
    ...
    <c:if test="${totalCount>0}">
        <div class="page" align="center" style="margin-top: 50px;"> 
            ...
            <c:if test="${startPage>1}">
                <a id="pagelbtn" href="market_tradeablesidetest?subtitle=${subtitle}&colorradio=${colorradio}&marketprice=${marketprice}&currentPage=${startPage-1}">
            ...
            <c:forEach var="pp" begin="${startPage}" end="${endPage}">
                <c:if test="${currentPage==pp}">
                    <a id="pagecnum" href="market_tradeablesidetest?subtitle=${subtitle}&colorradio=${colorradio}&marketprice=${marketprice}currentPage=${pp}"><b>${pp}</b></a>
                </c:if>
                <c:if test="${currentPage!=pp}">
                    <a id="pagenum" href="market_tradeablesidetest?subtitle=${subtitle}&colorradio=${colorradio}&marketprice=${marketprice}currentPage=${pp}">${pp}</a>
                </c:if>
            </c:forEach>
            
            <!-- 다음 -->
            <c:if test="${endPage<totalPage}">
                <a id="pagerbtn" href="market_tradeablesidetest?subtitle=${subtitle}&colorradio=${colorradio}&marketprice=${marketprice}currentPage=${endPage+1}">
